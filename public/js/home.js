...
$(document).on('scroll', function () {
    ...
    if (($(document).height() - $(document).scrollTop() - document.documentElement.clientHeight - 50) < 0) {
...
//举报----------------------------
var wsR = null, wReport = null,wCommets=null,wCommentsHeader=null;
// 扩展API加载完毕，现在可以正常调用扩展API
function plusReadyR() {
...
function showReport() {
    ...
    // 开启遮罩
    wsR.setStyle({mask: "rgba(0,0,0,0.5)"});
    // 创建侧滑页面
    wReport = plus.webview.create("report.html", "report", { bottom:0,height: 200, popGesture: "none"});
    // 侧滑页面关闭后关闭遮罩
    wReport.addEventListener('close', function () {
        wsR.setStyle({mask: "none"});
        wReport = null;
    }, false);
    // 侧滑页面加载后显示（避免白屏）
...
function showCommets(){
    ...
    // 创建侧滑页面
    wCommets = plus.webview.create("comments.html", "comments.html", {  popGesture: "close"});
    wCommentsHeader=plus.webview.create("comments_header.html", "comments_header", { top:0,height: 46, popGesture: "none",position:'absolute'});
